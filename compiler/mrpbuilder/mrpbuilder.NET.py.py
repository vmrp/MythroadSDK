...
# [GCC 8.3.0]
# Embedded file name: mrpbuilder.NET.py
# Compiled at: 2020-11-12 18:36:46
...
def dealcfiles(project_path, with_c_global=None, c_address=None, c_helper=None, localfilecahe=None, extname='cfunction', c_scatter=None, exram_fix=None, fileparams=[]):
        ...
        if c_address == None:
            if project['plat'] == 'mstar':
                ...
                if MR_C_Compiler != 'thumb':
                    param = param + ' -first mr_c_function_load'
                    param = param + ' -entry mr_c_function_load '
                elif bmp_mode == 2 or project['plat'] == 'via':
    ...
    if c_helper == True:
        ...
        elif project['plugin_now'] in ('app', 'appdev', 'appnpay'):
...
def compileextfile(extconfig, extname, project_path, fileparams, cpu):
    ...
    for f in files:
        ...
        filename = filename.replace('${MRPBUILDER_HOME}', MRPBUILDER_HOME)
...
def buildproject(projectfile):
    ...
    for config_real in project['config']:
        ...
        if project['plat'] == 'none':
            ...
            bmp_mode = 0
        print 'target platfrom:', project['plat']
        ...
        for include in include_configs:
            include = include.replace('${MRPBUILDER_HOME}', MRPBUILDER_HOME)
            AddInclude(include)
        ...
        if c_helper and (project['plugin'] == 'normal' or project['plugin'] == 'app' or project['plugin'] == 'appnpay' or project['plugin'] == 'main' or project['plugin'] == 'mainns'):
            ...
            if mr_authorization != 'mr':
                if project['plat'] == 'mtk':
                    ...
                    if project['loader'] in ('c', 'cbuf'):
                        mr_helper_filename = '\\mr_helperq.mrp'
                    else:
                        mr_helper_filename = '\\mr_helpere.mrp'
                else:
                    mr_helper_filename = '\\mr_helpere.mrp'
                ...
                if project['plat'] == 'spreadtrum' or bmp_mode == 2:
                    mr_helper_filename = '\\mr_helperp.mrp'
                else:
        ...
        for f in project['files']:
            ...
            filename_params = filename_params.replace('${MRPBUILDER_HOME}', MRPBUILDER_HOME)
            ...
            if filename[-4:] == '.mrp' and 'plugins.lst' in fileparams and 'packing' in fileparams:
                pluginf = open(filename)
                pluginf.seek(192)
                shortFileName = filename[filename.rfind('\\') + 1:]
                plugins_lst.append((shortFileName, pluginf.read(4), pluginf.read(4)))
                pluginf.close()
        ...
        if output_format_config == 'lib':
            ...
            if len(plugins_lst) > 0:
                data = ''
                print 'plugins summary:'
                for p in plugins_lst:
                    import struct
                    print '%20s' % p[0], 'appID:%10d' % struct.unpack('>I', p[1]), 'appVer:%10d' % struct.unpack('>I', p[2])
                    data += p[1]
                    data += p[2]

                zipdata = zipfile(data)
                addfile2cache('plugins.lst', zipdata, len(data))
...
MR_FROMELF = 'fromelf'
COMMAND_PATH = ''
print 'mrpBuilder(.NET) version:B01.05_20100809'
import sys
MRPBUILDER_HOME = path = sys.argv[0][0:sys.argv[0].rfind('\\') + 1]
print '${MRPBUILDER_HOME}:', MRPBUILDER_HOME
if MRPBUILDER_HOME.find('(') != -1:
    print "error:${MRPBUILDER_HOME} cannot contain '(' "
    sys.exit(-1)
if __name__ == '__main__':
    ...
    try:
        ...
        ret = auth_check.check_license()
        if ret == True:
            mr_authorization = auth_check.mr_authorization
            if AUTH_SUCP == True:
                mr_authorization = 'cp'
            g_host_auth_str = auth_check.g_host_auth_str
            main()
        else:
            print ret
            sys.exit(-1)