...
//文件系统测试
#define STR_FILEINFO "\x65\x87\x4e\xf6\x7c\xfb\x7e\xdf\x6d\x4b\x8b\xd5"
...
void main_FileInfo(int32 data)
{
	char path[256];
	uint16 *dir=NULL;
	int32 ret,*err=NULL,size;
	uint64 total;
	uint64 left;
	char RecBuf[70];
	
   DBG_PRINTF(("mrc_LCDCanSleep(0)=%d",mrc_LCDCanSleep(0)));
   DBG_PRINTF(("mrc_GetFreeSpace:%dbytes!",mrc_GetFreeSpace('C',NULL)));
   DBG_PRINTF(("mrc_GetDiskSpace:%dbytes!",mrc_GetDiskSpace('C',NULL)));
   mrc_GetDiskFreeSpaceEx((uint8 *)"C",&total,&left);
   DBG_PRINTF(("mrc_GetDiskFreeSpaceEx:total%d bytes!",total));
   DBG_PRINTF(("mrc_GetDiskFreeSpaceEx:left %d bytes!",left));
   ret=mrc_GetCurrentDirectory(256,path);
   if(ret!=0)
	   DBG_PRINTF(("mrc_GetCurrentDirectory:%s,len=%d",path,ret));
   else
	   	DBG_PRINTF(("mrc_GetCurrentDirectory Failed:ret=%d",ret));
   mrc_platTextNew(STR_FILEINFO, STR_FILEINFO, 
   	MR_DIALOG_CANCEL, 0, NULL, NULL);   
   dir=mrc_c2u(path,err,&size);
   DBG_PRINTF(("mrc_c2u return dir=%0x,size=%d",dir,size));
   if(dir)
	   mrc_platTextNew(STR_FILEINFO,(char *)dir, 
   			MR_DIALOG_CANCEL, 0, NULL, NULL); 
   mrc_memset(RecBuf,0,sizeof(RecBuf));
   mrc_GetMrpInfo("*A",MRP_APPID,(uint8 *)&ret,70);
   //mrc_GetMrpInfo(mrc_getPackName(),MRP_APPID,(uint8 *)&ret,4);   
   mrc_printf("MRP_APPID:%d",ret);
	return;
}
...
void main_ShowMenu(void)
{
   mrc_clearScreen(0, 0, 0);
	...
	main_menu=0;
	draw_WinHandle=0;
	main_menu = mrc_platMenuNew("\x5b\x9a\x65\xf6\x56\x68\x8c\x3\x5e\xa6",6);             // 定时器调度
	if (main_menu != MR_FAILED)
	{
	   ...
	   mrc_platMenuSetItem(main_menu, STR_FILEINFO, 4, main_FileInfo, 0);   // 帮助	   	   
	   mrc_platMenuSetItem(main_menu, "\x90\x0\x51\xfa\x6f\x14\x79\x3a", 5, main_exit, 0);   // 点击返回键，退出游戏         
	   mrc_platMenuSetItem(main_menu, "\x90\x0\x51\xfa\x6f\x14\x79\x3a", 6, main_exit, 0);   // 点击返回键，退出游戏         	   
